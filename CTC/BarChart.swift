...
//  CTC
//
//  Created by Aesha Patel on 2019-07-21.
...
class BarChartView: UIView {
    ...
    private func showEntry(index: Int, entry: BarEntry) {
        let xPos: CGFloat = translateWidthValueToXPosition(value:
            Float(entry.percentage) / Float(100.0))
        let yPos: CGFloat = space + CGFloat(index) * (barHeight + space)
        drawBar(xPos: xPos, yPos: yPos)
        drawTextValue(xPos: xPos + 155.0, yPos: yPos + 4.0, textValue: "\(entry.percentage)")
        drawTitle(xPos: 16.0, yPos: yPos + 12.0, width: 100.0, height:
            40.0, title: entry.practice)
    }
    private func drawBar(xPos: CGFloat, yPos: CGFloat) {
        let barLayer = CALayer()
        barLayer.frame = CGRect(x: 144.0, y: yPos, width: xPos, height: 40.0)
        barLayer.backgroundColor =  #colorLiteral(red: 0.2484704256, green: 0.8797343373, blue: 0.820797801, alpha: 1)
